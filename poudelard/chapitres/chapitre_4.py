...
def creer_equipe(maison, equipe_dict, est_joueur=False, joueur=None):
    joueurs_source = []
    if ("joueurs" in equipe_dict) and (type(equipe_dict["joueurs"]) is list):
        joueurs_source = list(equipe_dict["joueurs"])
    capitaine = None
    if "capitaine" in equipe_dict:
        capitaine = equipe_dict["capitaine"]
    equipe = {
        "nom": maison,
        "score": 0,
        "a_marque": 0,
    ...
    if est_joueur and (joueur is not None) and ("Prenom" in joueur) and ("Nom" in joueur):
        attrapeur = joueur["Prenom"] + " " + joueur["Nom"] + " (Attrapeur)"
        joueurs_sans_doublon = []
        i = 0
        while i < len(joueurs_source):
            if joueurs_source[i] != attrapeur:
                joueurs_sans_doublon.append(joueurs_source[i])
            i += 1
...
def afficher_equipe(maison, equipe):
    ...
    i = 0
    while i < len(equipe["joueurs"]):
        print(f"- {equipe['joueurs'][i]}")
        i += 1
...
def match_quidditch(joueur, maisons):
    ...
    while tour <= 20:
        ...
        if apparition_vifdor():
            ...
            print(f"{gagnante_capture['nom']} remporte le match...")
            actualiser_points_maison(maisons, gagnante_capture["nom"], 500)
            print(f"+500 points pour {gagnante_capture['nom']} !")
            return
